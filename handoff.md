...
- 2026-06-17T(Session 5): Shipped 作业批改 (workbook grading), 题库 (question bank), AI vision, audit fixes. Both platforms BUILD SUCCEEDED, 0 warnings. feature_list.json → 71 features all passing.
  • NEW 作业批改: subject-agnostic structured schema (Models/WorkbookGradingTypes.swift: GradedWorkbook/GradedQuestion/GradeVerdict/WorkbookQuestionType) handling math/语文/English/any subject; `gradeWorkbook` on IntelligenceService + all 3 impls. Cloud uses real multimodal vision (CloudChatClient.completeVision across openAI/anthropic/gemini dialects, gated by AIProvider.supportsVision) with OCR-text + offline-mock fallbacks (never throws to UI). Features/Workbook/ = WorkbookImageInput (camera iOS / Photos / file both platforms), WorkbookGradingView+Model, WorkbookResultView (WorkbookResultContent shared), WorkbookHistoryView. WorkbookGradeRecord persists photo+result+meta (批改历史 re-renders offline).
  • NEW 题库: BankedQuestion model + QuestionBankView/Model; save wrong/any question from 作业批改/拍题/错题本; 智能出题 generates 同类练习 via similarProblems seeded by selected questions. SampleData seeds 4.
  • Wiring: ToolKind.gradeWorkbook + .questionBank (AppEnums 4 switches + AppDestinations.toolView), Home tiles, macOS 学习 menu (作业批改 ⇧⌘G / 题库 ⇧⌘B). Real camera capture added to CaptureSolveView. Image now flows into SolveRequest for multimodal solve.
  • Audit fixes (34-agent parallel audit, verified-real only): robust extractJSON (balanced scan); DrillAnswerChecker/ExamAnswerChecker whole-part/token matching (fixed "10"≈"x=5,y=10"); EssayGradingView 升格范文 unlock gated on ParentControls.verified (closed learn-mode bypass); DocumentQA.ingest aborts on failed save; ConversationView streaming Task cancelled onDisappear; ModelContainerFactory graceful fallback (no try!); app-wide try? save() → saveLogging() (OSLog, Services/Persistence/PersistenceLog.swift); FoundationModelsService.chat route restamp; photo-pick failures give warning haptic; FoundationModels chat route.
  • Build repair: regenerated Services/Catalog/LessonContentCatalog.swift (69 lessons) via fixed scripts/generate_lesson_catalog.py — the working tree referenced this never-generated file (half-finished parallel feature) and didn't build clean. social.json still absent (falls back to MockContent).
